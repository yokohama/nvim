-- ============================================
-- Image Paste Utility (Cross-platform)
-- WSL: PowerShell, macOS: pngpaste
-- ============================================
...
local function has_image_in_clipboard()
  ...
  elseif is_wsl then
    ...
    vim.notify("Unsupported platform for image paste", vim.log.levels.ERROR)
    return false
  end
end
...
local function save_img(abs_path)
  if is_mac then
    -- macOS: pngpasteで保存
    local res = vim.system({ "pngpaste", abs_path }, { text = true }):wait()
    if res.code ~= 0 then
      vim.notify("Failed to save image via pngpaste\n" .. (res.stderr or ""), vim.log.levels.ERROR)
      return false
    end
    return true
